...
export class TransactionAutomationService {
     ...
     */
    #initializeServices() {
        const { GMAIL_OAUTH_CREDENTIALS, FIRESTORE_COLLECTION, CALENDAR_NAME } = this.config;

        // Check if we have all required services
        const hasAllServices = this.firestore && this.gmail && this.calendar && this.drive;
    ...
    async handleEvent(cloudEvent) {
        try {
            ...
            const docRef = this.firestore.collection(this.config.FIRESTORE_COLLECTION).doc(email);
        ...
        for (const { messagesAdded = [] } of res.data.history) {
            for (const { message } of messagesAdded) {
                try {
                    ...
                    const isNew = await checkAndMarkMessageProcessed(this.firestore, message.id);
                    if (!isNew) {
                    ...
                    const wasProcessed = await this.handleTransaction({ from, subject, message: msg });
                    if (wasProcessed) {
                        await this.markMessageAsRead({ from, subject, messageId: message.id });
                    }
                } catch (err) {
                    if (err.code === 404) {
    ...
    async handleTransaction({ from, subject, message }) {
        ...
        if (sender.includes('nationalgridus.com') && subj.includes('national grid bill')) {
            ...
            try {
                if (!this.nationalGrid) {
                    const { default: NationalGridClient } = await import('./NationalGridClient.js');
        ...
        if (sender.includes('capitalone.com') && subj.includes('withdrawal notice')) {
            ...
            if (body.includes('att has initiated')) {
                await this.processCalendarEvents('Pay AT&T', { action: 'delete', monthOffset: 0 });
                return true;
            } else if (body.includes('lowes has initiated')) {
                await this.processCalendarEvents('Pay Lowes', { action: 'delete', monthOffset: 0 });
                return true;
            } else if (body.includes('eastern bank has initiated')) {
                await this.processCalendarEvents('Pay Eastern Savings', { action: 'delete', monthOffset: 0 });
                return true;
            } else if (body.includes('sunrun has initiated')) {
                await this.processCalendarEvents('Sunrun withdrawal', { action: 'delete', monthOffset: 0 });
                return true;
            }
            return false;
        }
        ...
        if (sender.includes('americanexpress.com') && subj.includes('received your payment')) {
            ...
            await this.processCalendarEvents('Pay Amex', { action: 'delete', monthOffset: 1 });
            return true;
        }
        ...
        if (sender.includes('chase.com') && subj.includes('your credit card payment is scheduled')) {
            ...
            await this.processCalendarEvents('Pay Chase', { action: 'delete', monthOffset: 1 });
            return true;
        }
        ...
        if (sender.includes('chase.com') && subj.includes('you scheduled your mortgage payment')) {
            ...
            await this.processCalendarEvents('Pay mortgage', { action: 'delete', monthOffset: 1 });
            return true;
        }
        ...
        if (sender.includes('chase.com') && subj.includes('transaction with comcast / xfinity')) {
            ...
            await this.processCalendarEvents('Comcast / Xfinity Withdrawal', { action: 'delete', monthOffset: 0 });
            return true;
        }
        ...
        if (sender.includes('chase.com') && subj.includes('transaction with spi*eversource')) {
            ...
            await this.processCalendarEvents('Pay Gas Bill', { action: 'patch', monthOffset: 0, title: `Gas Bill - $${amount}` });
            return true;
        }
    ...
    async uploadToDrive(fileData, folderPath) {
        ...
        try {
            ...
            if (fileId) {
                ...
                console.log(`☁️ Overwrote existing "${fileData.fileName}" in Drive (ID: ${file.data.id})`);
            } else {
                // Create new file
                ...
                console.log(`☁️ Uploaded new "${fileData.fileName}" to Drive (ID: ${file.data.id})`);
            }
        } catch (error) {
    ...
    async processCalendarEvents(eventPrefix, { action, monthOffset = 0, title }) {
        ...
        try {
            ...
            const targetCal = calendarList.data.items.find(c => c.summary === this.calendarName);
            if (!targetCal) {
            ...
            for (const e of events) {
                if (action === 'delete') {
                    ...
                    await this.calendar.events.delete({ calendarId: targetCal.id, eventId: e.id });
