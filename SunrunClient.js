...
export default class SunrunClient {
    ...
    async getBillFromMessage(message, gmailService) {
        ...
        const pdfAttachment = await this.extractPdfAttachment(message, gmailService);
        if (!pdfAttachment) {
        ...
        let dateStr;
        try {
            const extractedDate = await this.extractBillDetailsFromPdf(pdfAttachment.data);
            if (extractedDate) {
    ...
    async extractBillDetailsFromPdf(pdfBuffer) {
        ...
        try {
            ...
            const billingPeriodRegex = /Billing Period[:\s]+([A-Za-z]{3}\s+\d{1,2}|\d{1,2}\/\d{1,2})\s*-\s*([A-Za-z]{3}\s+\d{1,2}|\d{1,2}\/\d{1,2})/i;
            ...
            const parseDatePart = (str) => {
                if (str.includes('/')) {
                    ...
                    return { month: monthMap[mStr.toLowerCase().substring(0, 3)], day: parseInt(dStr, 10) };
                }
            };

            const { month: billEndMonth, day: billEndDay } = parseDatePart(billingEndDateStr);
            const [dueMonth, dueDay, dueYear] = dueDateStr.split('/').map(Number);
